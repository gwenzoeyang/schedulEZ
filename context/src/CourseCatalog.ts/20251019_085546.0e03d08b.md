---
timestamp: 'Sun Oct 19 2025 08:55:46 GMT-0400 (Eastern Daylight Time)'
content_id: 0e03d08b5c7c8571219b002a89344e915f0ca6afe6ea762d650948c42a286b9a
---
...
type CourseDB = {
  ...
  meeting_times: Array<
    ...
    | string
  >;
  requirements?: string; // "CS111 || CS111M || ..."
...
function adaptCourse(dbRow: CourseDB): Course {
  const meetingTimes: TimeSlot[] = (dbRow.meeting_times ?? [])
    .map(normalizeTimeSlotLoose)
    .filter((t): t is TimeSlot => t !== null);

  const requirements: Requirement[] = parseRequirementsLoose(
    dbRow.requirements,
  );
...
// Helper: split "CS111 || CS112 or instructor" into [{code:"CS111"},...]
function parseRequirementsLoose(reqStr?: string): Requirement[] {
  if (!reqStr) return [];
  const tokens = reqStr
    .split(/\s*\|\|\s*|,|\s+or\s+/gi)
    .map((s) => s.trim())
